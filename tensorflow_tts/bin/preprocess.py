...
def preprocess():
    ...
    #remove the first element, has errors, its at index 386
    #train_split.pop(386)

    #make the txt bold for easy debugging, and with diff color
    bold_start = '\033[1m'
    bold_end   = '\033[0m'

    crestrt = '\033[44m'

    print('\033[4m', bold_start, crestrt, 'Debugging starts here')
    
    print('\n\t The number of training items \n\t{}\n'.format(len(train_split)))
  ...
  #This if for validation_set
    ...
    '''
    print('The total number of items worked on\n\t {}'.format(len(train_split_modified)))
    print('sample of items worked on\n\t {}'.format(train_split_modified[:5]))

    print('The total number of validation items worked on\n\t {}'.format(len(valid_split_modified)))
    print('sample of validation items worked on\n\t {}'.format(valid_split_modified[:5]))

    print('\033[4m', '\n\t Debuiggng stops here', bold_end)
    ...
    logging.info(f"Training items test: {len(train_split)}")
...
def gen_normal_mel(mel_path, scaler, config):
    ...
    suffix = path_split[-2]
    utt_id_split = file_name.split('-')
    utt_id =  utt_id_split[0] + '-' +  utt_id_split[1]
    np.save(
        os.path.join('./data/Preprocessed_data/dump/{}/'.format(suffix), "norm-feats", f"{utt_id}-norm-feats.npy"
        ),
...
def normalize():
    ...
    scaler.n_features_in_ = config["num_mels"]  
     #load raw-feats for both train and valid folders

    raw_feats = {}
    for np_name in glob.glob('./data/Preprocessed_data/dump/train/raw-feats/*.np[yz]'):
        raw_feats[np_name] = np.load(np_name)
    for np_name in glob.glob('./data/Preprocessed_data/dump/valid/raw-feats/*.np[yz]'):
        raw_feats[np_name] = np.load(np_name)

    mel_raw_feats = raw_feats
...
def compute_statistics():
    ...
    #load raw-f0
    raw_fo = {}
    for np_name in glob.glob('./data/Preprocessed_data/dump/train/raw-f0/*.np[yz]'):
        raw_fo[np_name] = np.load(np_name)

    #load raw-energies
    raw_energies = {}
    for np_name in glob.glob('./data/Preprocessed_data/dump/train/raw-energies/*.np[yz]'):
